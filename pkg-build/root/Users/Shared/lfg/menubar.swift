...
struct HelperConfig {
    ...
    struct SpotlightMonitorConfig {
        var enabled: Bool = true
        var cooldownMinutes: Int = 60
        var mdsThreshold: Double = 20.0
    }
    ...
    var spotlightMonitor = SpotlightMonitorConfig()
}
...
class HelperMonitor {
    ...
    // MARK: - mds CPU Monitor (SSD teal)

    func checkMdsCpu(delegate: LFGMenubar) {
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: NSHomeDirectory() + "/.config/lfg/state.json")),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let mods = json["modules"] as? [String: [String: Any]],
              let ssd = mods["ssd"],
              let mdsCpuStr = ssd["mds_cpu"] as? String,
              let mdsCpu = Double(mdsCpuStr) else { return }

        if mdsCpu > 50.0 {
            activeConditions.append("mds: \(String(format: "%.1f", mdsCpu))% CPU — critical")
            if shouldAlert(check: "mds_critical", cooldownMinutes: config.spotlightMonitor.cooldownMinutes) {
                delegate.sendNotification(title: "LFG SSD", body: "mds critical: \(String(format: "%.1f", mdsCpu))% CPU. Run: lfg ssd status")
                recordAlert(check: "mds_critical")
            }
        } else if mdsCpu > config.spotlightMonitor.mdsThreshold {
            activeConditions.append("mds: \(String(format: "%.1f", mdsCpu))% CPU — elevated")
            if shouldAlert(check: "mds_warn", cooldownMinutes: config.spotlightMonitor.cooldownMinutes) {
                delegate.sendNotification(title: "LFG SSD", body: "mds elevated: \(String(format: "%.1f", mdsCpu))% CPU. Consider: lfg ssd exclude")
                recordAlert(check: "mds_warn")
            }
        }
    }
...
class LFGMenubar: NSObject, NSApplicationDelegate {
    ...
    func buildResultSummary(name: String, info: [String: Any]) -> String {
        ...
        case "ssd":
            let mdsCpu = info["mds_cpu"] as? String ?? "?"
            let indexedCount = info["indexed_count"] as? String ?? "?"
            return "mds: \(mdsCpu)% · \(indexedCount) indexed"
        default:
    ...
    func buildMenu() {
        ...
        // --- SSD Actions Submenu ---
        let ssdItem = NSMenuItem(title: "SSD — Spotlight Monitor", action: nil, keyEquivalent: "")
        let ssdMenu = NSMenu()
        let ssdHeader = NSMenuItem(title: "SSD — Spotlight Monitor", action: nil, keyEquivalent: "")
        ssdHeader.isEnabled = false
        ssdMenu.addItem(ssdHeader)
        ssdMenu.addItem(NSMenuItem.separator())
        addSubItem(ssdMenu, "Status", action: #selector(ssdStatus), key: "")
        addSubItem(ssdMenu, "Scan Volumes", action: #selector(ssdScan), key: "")
        addSubItem(ssdMenu, "Exclude All (dry run)", action: #selector(ssdExclude), key: "")
        ssdItem.submenu = ssdMenu
    ...
    // SSD actions
    @objc func ssdStatus() { launchLFG("ssd status") }
    @objc func ssdScan() {
        sendNotification(title: "LFG SSD", body: "Scanning volumes...")
        launchLFG("ssd scan")
    }
    @objc func ssdExclude() {
        sendNotification(title: "LFG SSD", body: "Excluding all (dry run)...")
        launchLFG("ssd exclude")
    }
