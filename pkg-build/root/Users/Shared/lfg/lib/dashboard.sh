...
# --- SSD: Spotlight status ---
echo "Checking Spotlight..."
SSD_MDS_CPU=$(ps aux 2>/dev/null | awk 'NR>1 && $11~/mds/{s+=$3} END{printf "%.1f%%", s+0}')
SSD_INDEXED=$(mdutil -a -s 2>/dev/null | grep "Indexing enabled" | wc -l | tr -d ' ')
SSD_ROWS=""
while IFS= read -r vol; do
    vol_path="${vol%/}"
    vol_name=$(basename "$vol_path")
    status=$(mdutil -s "$vol_path" 2>/dev/null | grep -i "indexing" | head -1 || echo "")
    if echo "$status" | grep -qi "enabled"; then
        badge='<span class="status-badge badge-error">indexed</span>'
    else
        badge='<span class="status-badge badge-cleaned">excluded</span>'
    fi
    SSD_ROWS+="<tr><td class=\"name\">${vol_name}</td><td>${badge}</td><td class=\"meta\">${vol_path}</td></tr>"
done < <(
    for vol in /Volumes/*/; do [[ -d "$vol" ]] && basename "$vol" | grep -qvE "^(Macintosh HD|Macintosh HD - Data|Recovery)$" && echo "$vol"; done
    for vol in "$HOME/Library/CloudStorage/"/*/; do [[ -d "$vol" ]] && echo "$vol"; done
)
[[ -z "$SSD_ROWS" ]] && SSD_ROWS='<tr><td colspan="3" class="empty-state">No external volumes found.</td></tr>'
SSD_STAT="${SSD_MDS_CPU} / ${SSD_INDEXED} indexed"
...
<style>{theme}</style>
  ...
  <div class="summary">
    ...
    <div class="stat clickable" onclick="switchTab('ssd')" data-tip="Click to view Spotlight status">
      <span class="label">SSD</span><span class="value" style="color:#facc15">SSD_STAT_PH</span>
    </div>
  </div>
  <div class="nav">
    ...
    <a onclick="switchTab('ssd')">SSD <span class="kbd">&#x2318;6</span></a>
  </div>
  ...
  <div id="tab-ssd" class="tab-content">
    <div class="section-title" style="color:#facc15">Spotlight Status</div>
  ...
  document.addEventListener('keydown', function(e) {{
    ...
    if (e.metaKey && e.key === '6') switchTab('ssd');
  }});
...
html = html.replace('SSD_ROWS_PLACEHOLDER', '''$SSD_ROWS''')
open('$HTML_FILE', 'w').write(html)
"
